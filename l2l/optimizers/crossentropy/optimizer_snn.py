...
CrossEntropyParametersSNN = namedtuple('CrossEntropyParameters',
                                       ['pop_size', 'rho', 'smoothing', 'temp_decay', 'n_iteration', 'distribution',
                                        'stop_criterion', 'n_batches', 'seed'])

CrossEntropyParametersSNN.__doc__ = """
...
class CrossEntropyOptimizerSNN(Optimizer):
    """
    ...
    def __init__(self, traj, optimizee_prepare,
                 optimizee_create_individual, optimizee_fitness_weights, parameters,
        ...
        # Prepare individual to connect the network
        size_eeo, size_eio, size_ieo, size_iio = self.optimizee_prepare()
        temp_indiv, self.optimizee_individual_dict_spec = dict_to_list(self.optimizee_create_individual(size_eeo,
                                                                                                        size_eio,
                                                                                                        size_ieo,
                                                                                                        size_iio),
                                                                       get_dict_spec=True)
        ...
        traj.f_add_derived_parameter('n_elite',
                                     int(parameters.rho * parameters.pop_size),
        ...
        # optimizee_create_individual creates individuals)

        # Note that this array stores individuals as an np.array of floats
        # as opposed to Individual-Dicts
        # This is because this array is used within the context of the
        # cross entropy algorithm and
        ...
        if optimizee_bounding_func is not None:
            current_eval_pop = [self.optimizee_bounding_func(
                ind) for ind in current_eval_pop]
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        weights = [np.concatenate(
            (individuals[i].weights_eeo, individuals[i].weights_eio,
             individuals[i].weights_ieo, individuals[i].weights_iio)) for i in
            range(traj.pop_size)]
        ...
        sorted_fitness = np.asarray(weighted_fitness_list)[
            fitness_sorting_indices]

        # Elite individuals are with performance better than or equal to the
        # (1-rho) quantile.
        ...
        if temp_decay > 0:
            # Keeping non-elite samples with certain probability dependent on
            # temperature (like Simulated Annealing)
            ...
            non_elite_selected_indices = self.random_state.binomial(
                1, non_elite_selection_probs).astype(bool)
        ...
        if self.g < n_iteration - 1 and self.best_fitness_in_run < stop_criterion:
            ...
            for ind_asarray in self.eval_pop_asarray:
                res_dict = list_to_dict(ind_asarray, self.optimizee_individual_dict_spec)
                res_dict['targets'] = self.optimizee_labels
                res_dict['train_set'] = self.train_set
                self.eval_pop.append(res_dict)
            # self.eval_pop = [list_to_dict(ind_asarray, self.optimizee_individual_dict_spec)
            #                  for ind_asarray in self.eval_pop_asarray]
            # Clip to boundaries
            if self.optimizee_bounding_func is not None:
                self.eval_pop = [self.optimizee_bounding_func(
                    individual) for individual in self.eval_pop]
                self.eval_pop_asarray = np.array(
                    [dict_to_list(x) for x in self.eval_pop])
